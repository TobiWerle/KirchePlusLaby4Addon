...
public class SheetHandler {
    ...
    public static void getMemberOwnSheet() throws IOException {
        ...
        for(Sheet sheet : sheets){
            ...
            if(Arrays.stream(get).anyMatch(name::contains) && !name.toLowerCase().contains("frei")){
                if(name.contains(Laby.labyAPI().minecraft().getClientPlayer().getName())){
                    MemberSheet = sheet;
    ...
    public static void saveActivity(activityTypes type) throws IOException {
        KirchePlus.main.utils.displayMessage("§bDeine Aktivität wird eingetragen...");
            ...
            public void run() {
              ...
              //  }
                ...
                if(screenshot.equals("Nicht erstellt!")){
                    KirchePlus.main.utils.displayMessage("§cDer Screenshot konnte nicht hochgeladen werden! Versuche es erneut!");
                    return;
                }
                ...
                if(type == activityTypes.TAFEL || type == activityTypes.SHG || type == activityTypes.SPENDEEVENT || type == activityTypes.BEICHTEVENT ){
                    ...
                    try {
                        ...
                        KirchePlus.main.utils.displayMessage("§cEs ist ein unerwarteter Fehler aufgetreten!");
    ...
    private static void tryUpdateSheetName(String error, activityTypes type) {
        if(error.contains("Unable to parse range")){
            if(fixtrys == 1){
                KirchePlus.main.utils.displayMessage("§eDer Fehler wurde nicht gefunden. Ein Neustart des Spiels wird dies beheben.");
                return;
            }
            fixtrys = 1;
            try {
                KirchePlus.main.utils.displayMessage("§eEs wird versucht die Aktivität erneut einzutragen...");
                getMemberOwnSheet();
                saveActivity(type);
            }catch (Exception e){
                e.printStackTrace();
    ...
    private static void writeEventActivity(activityTypes type, List<String> ranges, List<List<Object>> values) throws IOException {
        ...
        if(valueRange == null){
            KirchePlus.main.utils.displayMessage("§cEs wurde nicht genug Platz in deinem Aktinachweis gefunden!");
            return;
        }
        ...
        KirchePlus.main.utils.displayMessage("§bDeine Aktivität wurde eingetragen!");
    }
