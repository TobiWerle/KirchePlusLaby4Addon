...
public class TabellenMethoden {
    ...
    public static void getHVList() throws IOException, GeneralSecurityException {
        ...
        if (values2 == null || values2.isEmpty()) {
            ...
            for (List row : values2) {
                try {
                    new HV_User(row.get(0).toString(), row.get(1).toString(), row.get(2).toString(),
                        row.get(4).toString(), "Nie", "Permanent");
                } catch (Exception ignored) {
                }
            }
        }
    ...
    public static void getBrotList() throws IOException, GeneralSecurityException {
        ...
        if (values == null || values.isEmpty()) {
            ...
            for (List row : values) {
                try {
                    new Brot_User(row.get(0).toString(), row.get(1).toString(),
                        row.get(2).toString());
                } catch (Exception e) {
                }
            }
                ...
                if (list1 != null) {
                    for (List row : list1) {
                        if (SpenderInfo.exists(row.get(0).toString())) {
                            SpenderInfo.getByName(row.get(0).toString())
                                .addAmount(Integer.valueOf(row.get(1).toString()).intValue());
                            continue;
                        }
                        KirchePlus.main.spender.add(
                            new SpenderInfo(row.get(0).toString(), row.get(1).toString()));
                    }
                }
    ...
    public static void getDonations() throws IOException {
        ...
        if (values == null || values.isEmpty()) {
            ...
            for (List row : values) {
                try {
                    SpenderUtils.publicDonations.add(
                        new publicDonators(row.get(0).toString(), row.get(1).toString(),
                            Integer.parseInt(
                                row.get(2).toString().replace("$", "").replace(".", ""))));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public static List<List<Object>> getList(String range) throws IOException {
        ValueRange response2 = sheetsService.spreadsheets().values().get(SPREADSHEET_ID, range)
            .execute();
    ...
    public static boolean isSameDay(String s) {
        ...
        if (date.length == 2) {
            if (Integer.parseInt(date[0]) == currentday
                && Integer.parseInt(date[1]) == currentmonth) {
                return true;
            }
        }
        if (date.length == 3) {
            if (Integer.parseInt(date[0]) == currentday && Integer.parseInt(date[1]) == currentmonth
                && Integer.parseInt(date[2]) == currentyear) {
                return true;
            }
        }
    ...
    public static void getActivitys() throws IOException {
        KirchePlus.main.totalActivity.clear();
        ...
        if (values == null || values.isEmpty()) {
                ...
                try {
                    if (!row.get(1).toString().toLowerCase().contains("frei")) {
                        Activity_User user = new Activity_User(
                            row.get(1).toString().replace("[", "").replace("]", "").replace(" ", "")
                                .replace("L", ""), row.get(6).toString(), row.get(5).toString(),
                            row.get(7).toString());
            ...
            KirchePlus.main.totalActivity = total.entrySet().stream()
